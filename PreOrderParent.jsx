...
/* ============================================================
   ...
   REVISI: WhatsApp WAJIB. Tambah Tingkat (Kelas opsional utk
   Guru/Karyawan). Waktu Ambil tetap tampil, DEFAULT "Istirahat 1".
   "Sebelum masuk" dibuang (hampir tak ada peminat — kata mama).
   ============================================================ */
...
export default function PreOrderParent({
  kantin = "Kantin Gan En", serviceDate = "Jumat, 27 Juni 2026",
  open = true, menus = MENUS, pickupOptions = PICKUP, onSubmit,
}) {
  ...
  const submit = () => {
    ...
    const kelasLabel = kelasNeeded ? `${form.tingkat} · ${form.kelas}` : form.tingkat;
    const r = {
      no: "PO-" + uid(), nama: form.nama, tingkat: form.tingkat, kelas: kelasNeeded ? form.kelas : "",
      kelasLabel, wa: form.wa, ambil: form.ambil, serviceDate, submittedAt: new Date().toLocaleString("id-ID"),
    ...
    return (
      <Screen>
        <div style={{ padding: "32px 20px" }}>
          ...
          <button onClick={() => { setCart({}); setForm({ nama: "", tingkat: "", kelas: "", wa: "", ambil: DEFAULT_PICKUP }); setTried(false); setStep("landing"); }}
...
function Stepper({ val, onMinus, onPlus }) {
  ...
  return <div className="flex items-center gap-2" style={{ background: t.surfaceSoft, border: `1px solid ${t.border}`, borderRadius: 999, padding: 3 }}>
    ...
    <button onClick={onPlus} style={b}><Plus size={18} /></button></div>;
}
