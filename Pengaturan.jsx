...
import {
  Store, Printer, MessageCircle, Database, Info, X,
  ChevronRight, Check, Download,
} from "lucide-react";

/* ============================================================
   PENGATURAN — Canteen Gan En  (dibuka dari ikon gear di header)
   Decision Lock: HANYA config jarang berubah —
   Nama Kantin, Printer, Nomor WhatsApp, Backup, Tentang.
   Status Pre-order & Tanggal Layanan TIDAK di sini (itu operasional,
...
const t = {
  ...
  error: "#D95D5D", border: "#E8E0D2", divider: "#F1EBE0", amberText: "#9A6700",
};

export default function Pengaturan({ onClose }) {
  const [nama, setNama] = useState("Kantin Gan En");
        ...
        <div style={{ padding: "0 20px" }}>
          ...
          <Group label="Kantin">
            <FieldRow icon={<Store size={20} />} label="Nama Kantin">
              <input value={nama} onChange={(e) => setNama(e.target.value)} style={inp} />
            </FieldRow>
            <FieldRow icon={<MessageCircle size={20} />} label="Nomor WhatsApp">
              <input value={wa} onChange={(e) => setWa(e.target.value)} placeholder="08…" style={inp} />
            </FieldRow>
          </Group>
          ...
          <Group label="Perangkat">
            <div className="flex items-center gap-3" style={{ padding: "14px 16px" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12.5, color: t.text2, marginTop: 2 }}>{printer ? "Terhubung" : "Belum terhubung"}</div>
              </div>
              <button onClick={() => { setPrinter(!printer); setToast(printer ? "Printer diputus" : "Printer terhubung"); }}
                className="flex items-center gap-1" style={{ height: 40, padding: "0 14px", borderRadius: 10, border: `1.5px solid ${printer ? "#D8E6D4" : t.border}`, background: printer ? t.successBg : t.surface, color: printer ? t.successText : t.text, fontWeight: 700, fontSize: 13.5, cursor: "pointer" }}>
                {printer ? <><Check size={16} /> Aktif</> : "Hubungkan"}
              </button>
            </div>
          </Group>
...
const ic = { width: 42, height: 42, borderRadius: 11, background: t.primaryLight, color: t.amberText, display: "grid", placeItems: "center", flex: "none" };
const inp = { width: "100%", height: 46, fontSize: 16, color: t.text, background: t.surface, border: `1.5px solid ${t.border}`, borderRadius: 10, padding: "0 12px", outline: "none", fontFamily: "inherit" };
const rowBtn = { width: "100%", padding: "14px 16px", background: "transparent", border: "none", cursor: "pointer", color: t.text };
    ...
    <div style={{ marginBottom: 18 }}>
      ...
      <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 2px rgba(47,42,36,.04)" }}>
        {React.Children.map(children, (c, i) => (
          <div style={i > 0 ? { borderTop: `1px solid ${t.divider}` } : undefined}>{c}</div>
        ))}
      </div>
    </div>
  );
